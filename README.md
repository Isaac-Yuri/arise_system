...
- [ ] **PASSO 2: VISUAL DOS CARDS DE TAREFA & RECOMPENSAS**
  ...
  - [ ] Criar mapeamento de cores neon no Tailwind baseado no Rank da tarefa (Zinco para Rank E, Vermelho para Rank A, Dourado com pulso para Rank S).

- [ ] **PASSO 3: SISTEMA DE RANKS PARA O CAÇADOR (PLAYER RANKS)**
  - [ ] Adicionar a coluna `rank` na tabela `users` do Supabase (Padrão inicial: `E`).
  - [ ] Implementar a lógica de evolução de Rank por faixas de nível na função de Level Up do `Dashboard.tsx` (Ex: Rank D no Nível 10, Rank S no Nível 70+).
  - [ ] Atualizar o `ProfileHeader.tsx` para exibir o Rank atual ao lado do nível (Ex: `LVL 12 · RANK D`).

- [ ] **PASSO 4: CORES DINÂMICAS DO APP CONFORME O RANK DO CAÇADOR**
  - [ ] Mapear as auras blur de fundo do aplicativo com base no Rank do jogador carregado do banco.
  - [ ] Tornar a atmosfera visual responsiva (o app inteiro muda de tom conforme o Hunter se torna mais poderoso).
